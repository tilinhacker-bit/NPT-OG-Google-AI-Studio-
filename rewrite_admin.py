...
# Replace the whole adminStats useMemo and the return statement
new_component = """import React, { useMemo } from "react";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {groups.map((g) => {
          ...
          return (
            ...
            >
              <h3 className={`text-sm font-black mb-3 uppercase tracking-wider flex items-center gap-1.5 ${isUserGroup ? "text-pink-600" : "text-slate-800"}`}>
                <Activity className={`h-4 w-4 ${isUserGroup ? "text-pink-500" : "text-indigo-500"}`} /> 
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {(["Duty", "Off", "Pre", "Ord", "Rest", "Anes"] as const).map(
                      (role) => (
